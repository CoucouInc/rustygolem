...
impl std::convert::TryFrom<u8> for Month {
    ...
    fn try_from(value: u8) -> Result<Self, Self::Error> {
        if (0..=12).contains(&value) {
            ...
            Err(anyhow!("Month out of bound: {}", value))
        }
    }
}

#[derive(Eq, PartialEq, Debug)]
pub(crate) struct RepublicanDate {
    pub year: i32,
    pub month: Month,
    pub day: u8,
}
...
impl std::fmt::Display for RepublicanDate {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            ...
            self.month,
            self.year,
            day_symbol(self.month, self.day),
            day_name(self.day)
        )
    }
}

fn day_symbol(m: Month, day: u8) -> &'static str {
    use Month::*;
    match (m, day) {
        (Vnd, 1) => "du raisin",
        (Vnd, 2) => "du safran",
...
impl RepublicanDate {
    fn from_yd(y: i32, day_of_year: i64) -> Result<Self> {
        let raw_m = day_of_year / 30;
        let month =
            Month::try_from(raw_m as u8).context(format!("cannot find a month for {}", raw_m))?;
...
impl std::convert::TryFrom<NaiveDate> for RepublicanDate {
    type Error = anyhow::Error;

    fn try_from(value: NaiveDate) -> Result<Self, Self::Error> {
        let french_era_end = NaiveDate::from_ymd(1811, 9, 23);
        let duration_since_french_era_end = value - french_era_end;
        if duration_since_french_era_end.num_seconds() < 0 {
            return Err(anyhow!(
                "Can only convert date from after the official end of the calendar: {}",
                french_era_end
            ));
        }
        ...
        // artificially pad it.
        let padding = 2000;
        let fake_french_date = NaiveDate::from_ymd(20 + padding, 1, 1);
        let french_date = fake_french_date + duration_since_french_era_end;
        let tmp_date = french_date
            .clone()
            .with_month0(0)
            .unwrap()
            .with_day0(0)
            .unwrap();
        let day_of_year = (french_date - tmp_date).num_days();
